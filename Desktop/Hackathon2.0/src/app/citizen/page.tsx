...
export default function CitizenPage() {
  const { demoDocument: { status, citizenName: name, shareCode }, scanDocument: uploadPassport } = useDemoStore();
  ...
  function handleCopy() {
    const formatted = shareCode.slice(0, 3) + "-" + shareCode.slice(3);
      ...
      <AnimatePresence mode="wait">
        {status === "UNUPLOADED" && (
          ...
          >
            <div className="w-full rounded-3xl border border-charcoal/[0.06] bg-white/70 p-8 text-center shadow-card backdrop-blur-xl">
              ...
              <h2 className="font-display text-lg font-semibold text-charcoal">Add your Passport</h2>
              <p className="mt-1 text-[13px] text-charcoal/40">
                Scan your passport to get verified instantly
              </p>
            </div>
            ...
            <AnimatePresence mode="wait">
              {scanning ? (
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="flex w-full flex-col items-center gap-4 rounded-3xl border border-atlas/20 bg-atlas/5 px-8 py-8 backdrop-blur-xl"
                >
                  ...
                  <div className="relative h-20 w-44 overflow-hidden rounded-xl border-2 border-atlas/30 bg-atlas/[0.04]">
                    <motion.div
                      ...
                      className="absolute inset-x-0 h-0.5 bg-gradient-to-r from-transparent via-atlas to-transparent shadow-glow"
                    />
                    <div className="absolute inset-2 flex items-center justify-center">
                      <ScanLine className="h-8 w-8 text-atlas/30" />
                    </div>
                  </div>
                  <p className="font-display text-sm font-medium text-atlas">Scanning document…</p>
                </motion.div>
              ) : (
                <motion.button
                  key="btn"
                ...
                >
                  ...
                  <span className="font-display text-[16px] font-bold text-white">Scan Passport</span>
                </motion.button>
              )}
            </AnimatePresence>
          </motion.div>
        ...
        {status === "PENDING_GOV" && (
          ...
          >
            ...
            <h2 className="font-display text-lg font-semibold text-charcoal">Under Verification</h2>
            <p className="mt-2 text-[13px] leading-relaxed text-charcoal/40">
              Your passport is being verified by SmartGov. This usually takes a few moments.
            </p>
            {/* Pulse indicator */}
            <div className="mt-6 flex items-center justify-center gap-2">
              <span className="h-2 w-2 animate-pulse rounded-full bg-sahara" />
              <span className="text-[12px] font-medium text-sahara">PENDING REVIEW</span>
            </div>
          </motion.div>
        )}
        ...
        {status === "VERIFIED" && (
          ...
          >
            ...
            <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-atlas via-atlas to-atlas-dark p-6 shadow-card">
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-0.5 text-[11px] font-semibold text-white/70">PASSPORT</p>
                </div>
              ...
              <div className="mt-5 flex items-end justify-between">
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-white/40">Expires</p>
            ...
            <div className="rounded-3xl border border-charcoal/[0.06] bg-white/70 px-6 py-6 shadow-card backdrop-blur-xl">
              <p className="mb-3 text-[11px] font-semibold uppercase tracking-wider text-charcoal/35">
                Share Code
              </p>
              <div className="flex items-center justify-between">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.92 }}
                  onClick={handleCopy}
                  className={`flex items-center gap-1.5 rounded-2xl px-4 py-2.5 text-[12px] font-semibold transition-colors ${
                    copied
                      ? "bg-atlas/10 text-atlas"
              ...
              <p className="mt-3 text-[12px] text-charcoal/35">
                Share this code with a government agent or third-party verifier.
              </p>
            </div>
          </motion.div>
        )}
